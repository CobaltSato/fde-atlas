...
type: 必殺技
sor: checklists/regression-cases.md・fixtures/・context/registry.md
...
## 手順
1. 交代内容を確認する(旧→新、理由)。
2. checklists/model-change.md を上から順に実行する(回帰→縮退切替→自律度-1→クセ追記→上限見直し→registry記録)。
3. 回帰は checklists/regression-cases.md × fixtures/ で再実行し、「期待される出力の要点」と比較して差分を表で報告する。
4. AGENTS.md記入欄の運転モード・自律度を更新する(これが唯一書き換えてよいAGENTS.md箇所)。
5. 完了報告する(回帰n/n合格、変わった点)。

## ゲート(Yes/Noで自己確認)
- [ ] regression-cases.mdの全ケースを再実行し差分を表で報告したか
...
## 完了条件
regression-cases.mdの全ケースの結果が記録され、AGENTS.mdの運転モード・自律度が更新され、registry.mdの交代履歴へ1行追記されている。
