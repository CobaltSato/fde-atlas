...
fde-atlas/scripts/install.sh ~/仕事のフォルダ    # 新規フォルダとして作成します(既存フォルダには導入できません)
cd ~/仕事のフォルダ
...
`install.sh` は**新規フォルダ作成専用**です。指定したパスが既に存在するとエラーで止まり、何もコピーしません(既存のプロジェクトに中途半端に混ざるのを防ぐため)。既存の仕事フォルダで使いたい場合は、まず別の新規フォルダに導入してから、そのフォルダの中身(`.claude/`・`AGENTS.md`・`fde-guide.md` など)を手動でコピー/参照してください。合わなかったら導入先フォルダごと削除すれば、跡形なく元に戻せます。
